...
class Solution:
    def shortestCompletingWord(self, licensePlate: str, words: List[str]) -> str:
        def transform_str(my_str: str):
            res = []
            digits_and_space = "0123456789 "
            my_str = my_str.lower()
            for char in my_str:
                if char in digits_and_space:
                    continue
                else:
                    res.append(char)
            return res
        ...
        res = []
        for word in words:
            mod_license_plate = transform_str(licensePlate)
            for char in word:
                if char in mod_license_plate and mod_license_plate:
                    mod_license_plate.remove(char)
                    if len(mod_license_plate) == 0:
                        res.append(word)
                elif char not in mod_license_plate:
                    continue
        if len(res) == 1:
            return res[0]
        else:
...
sol = Solution()
for el in arr:
    print(sol.shortestCompletingWord(el[0], el[1]))
